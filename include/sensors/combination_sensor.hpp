...
class CombinationSensor : public IAngleSensor
{
   public:
    ...
    struct Params
    {
        float max_drift_deg{0};  ///< Absolute maximum drift the primary sensor
                                 ///< can have before needing to be reset
    };
    ...
    /**
     * @brief Reads the angle of the sensor and recalibrates the primary sensor
     * if needed
     *
     * @return std::optional<float> The current angle reported by the
   ...
   private:
    ...
    IAngleSensor *p_primary_sensor_;  ///< Pointer to the primary sensor used by
                                      ///< the device
    IAngleSensor
        *p_reference_sensor_;  ///< Pointer to the reference sensor used
